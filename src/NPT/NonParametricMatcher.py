...
class StereoMatcher(object):
    ...
    def local_match(self, i, j):
        ...
        if i < self.x_range[0] or i > self.x_range[1] or j < self.y_range[0] or j > self.y_range[1]:
            logging.debug('Match %d,%d  out of range %d, %d, %d, %d, continuing', i,j, self.x_range[0], self.x_range[1], self.y_range[0], self.y_range[1])
            return    
...
class NonParametricMatcher(StereoMatcher):
    def __init__(self, image1, image2, x_range = None, y_range = None, agg_rad = 3, null_value = -999):
        StereoMatcher.__init__(self, image1, image2, x_range = x_range, y_range = y_range, agg_rad = agg_rad, null_value = null_value)
...
class NonParametricMatcherSubPixel(NonParametricMatcher):
    def __init__(self, image1, image2, x_range = None, y_range = None, agg_rad = 3, null_value = -999):
        NonParametricMatcher.__init__(self, image1, image2, x_range = x_range, y_range = y_range, agg_rad = agg_rad, null_value = null_value)
        ...
        self.stack = numpy.zeros([self.output_shape[0], self.output_shape[1], evaluations], numpy.float32) #@UndefinedVariable
    ...
    def subpixel_values(self):
        ...
        y_index = numpy.lib.stride_tricks.as_strided(y_index, (self.y_displacement.shape[1], y_index.size), (0, y_index.itemsize)).T
        x_index = numpy.arange(self.y_displacement.shape[1])
        x_index = numpy.lib.stride_tricks.as_strided(x_index, (self.y_displacement.shape[0], x_index.size), (0, x_index.itemsize))
        
        #find the maximum in the neighbourhood of each WTA match  
        maxima = numpy.zeros(self.y_displacement.shape)
        iterative_index =  minima_index - 2;  oob = minima_index < 0;  minima_index[oob] = 0
        for i in xrange(5):
            ...
            iterative_index += 1
            oob = iterative_index < 0;  iterative_index[oob] = 0
            oob = iterative_index > max_index;  iterative_index[oob] = max_index
        ...
        for i in xrange(5):
            ...
            oob = shifted_minimas < 0;  shifted_minimas[oob] = 0
            oob = shifted_minimas > max_index;  shifted_minimas[oob] = max_index
